...
export default function ContactsPage() {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`Are you sure you want to delete "${name}"? This will also delete their outreach history.`)) {
      return;
    }
    ...
    <div className="min-h-screen p-8 max-w-4xl mx-auto">
      ...
      {contacts.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 dark:bg-gray-800 rounded-lg">
          <p className="text-xl text-gray-600 dark:text-gray-400">No contacts yet</p>
          {accounts.length > 0 ? (
            <button
              onClick={() => setShowForm(true)}
        ...
        <div className="space-y-3">
          <p className="text-gray-600 dark:text-gray-400">
            {contacts.length} contact{contacts.length !== 1 ? 's' : ''} across {accounts.length} account{accounts.length !== 1 ? 's' : ''}
          </p>
